...
const Redis = require('d:/Baalvion Projects/Backend/services/commerce/order-service/node_modules/ioredis');

const OS = Number(process.env.OS_PORT) || 3113;
const RB = 3055;
...
(async () => {
  ...
  await redis.set('auth:blacklist:' + jti, '1', 'EX', 3600);
  ...
  await redis.del('auth:blacklist:' + jti); await redis.quit();
  console.log('\n=== DONE ===');
