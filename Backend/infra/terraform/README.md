# Baalvion Production AWS Infrastructure (Terraform)
...
`main.tf` uses an S3 backend with DynamoDB locking. The bucket and lock table
must exist **before** `terraform init`. Create them once:
...
## 2. Required secret inputs (env vars — never put these in tfvars)
...
`environment = "production"`, `redis_auth_token` is enforced by a precondition
(it gates `transit_encryption_enabled` on the Redis cluster).
...
- **EKS API server** currently allows `0.0.0.0/0` (see `modules/eks/main.tf`).
  **Restrict `public_access_cidrs` to your office / VPN / CI egress ranges for
...
- **RLS role**: this stack does NOT provision the `baalvion_app` DB role or run
  app migrations — do that during app deploy (see `Backend/docs/RLS-PRODUCTION-CUTOVER.md`).
...
provider alias (not yet wired) — keep disabled until the multi-region rollout.

## 7. Outputs operators use

`eks_cluster_name`, `db_endpoint`, `redis_endpoint`, `alb_dns_name`,
`alb_zone_id` (Route53 alias), `cloudfront_domain_name`, `ecr_repository_urls`,
`secret_arns`, `ssm_parameter_arns`, `secrets_read_policy_json`,
`eks_node_security_group_id`, `waf_web_acl_arn`, `kubeconfig_command`.
