...
function fetchJwks() {
  const now = Date.now();
  if (_jwksKeys && (now - _jwksCachedAt) < config.jwt.jwksTtlMs) {
    return Promise.resolve(_jwksKeys);
  }

  return new Promise((resolve, reject) => {
    const uri = config.jwt.jwksUri;
    const mod = uri.startsWith('https') ? https : http;
    const req = mod.get(uri, { timeout: 5000 }, (res) => {
      let body = '';
      res.on('data', (chunk) => { body += chunk; });
      res.on('end', () => {
        if (res.statusCode !== 200) {
          return reject(new Error(`JWKS fetch HTTP ${res.statusCode}`));
        }
        try {
          const parsed  = JSON.parse(body);
          _jwksKeys      = parsed.keys;
...
/**
 * Verify a JWT token. Tries in order:
 *  1. RS256 via JWKS (preferred — auth-service asymmetric keys)
 *  2. RS256 via static public key env var (dev / air-gapped fallback)
 *  3. HS256 via JWT_SECRET (shared secret — proxy-backend / jobs-service tokens)
 */
async function verifyWsToken(token) {
  ...
  if (alg === 'RS256' || !alg) {
    try {
      ...
      // ── Path 2: Static RSA public key ─────────────────────────────────────
      const raw = config.jwt.publicKey
        || (config.jwt.publicKeyB64
            ? Buffer.from(config.jwt.publicKeyB64, 'base64').toString('utf8')
            : null);
      if (raw) {
        try {
      ...
      // Re-throw JWKS error unless HS256 secret is available
      if (!config.jwt.secret) throw jwksErr;
    }
  }

  // ── Path 3: HS256 shared secret ────────────────────────────────────────────
  if (!config.jwt.secret) throw new Error('No valid JWT verification method available');
...
function createAuthMiddleware(namespacePath) {
  ...
  return async (socket, next) => {
    ...
    try {
      ...
      socket.data.orgId       = payload.org_id   ?? payload.orgId ?? null;
      socket.data.role        = payload.role      ?? 'member';
      socket.data.permissions = payload.permissions || [];
      socket.data.sessionId   = payload.sid       ?? null;

      // Namespace role check
      if (allowedRoles.length && !allowedRoles.includes(socket.data.role)) {
...
function attachConnectionHandler(namespace, namespacePath) {
  ...
  namespace.on('connection', async (socket) => {
    const { userId, orgId, role } = socket.data;
    const ip = socket.handshake.address;
    console.log(`[WS] Connected — ns=${namespacePath} user=${userId} role=${role} ip=${ip}`);
