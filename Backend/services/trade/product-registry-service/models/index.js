...
db.HsDocRequirement = require('./hs_doc_requirements')(sequelize, Sequelize.DataTypes);
Object.values(db).forEach((m) => { if (m && m.associate) m.associate(db); });

// R1 GUC bridge — set app.current_tenant / app.tenant_bypass on every transaction.
