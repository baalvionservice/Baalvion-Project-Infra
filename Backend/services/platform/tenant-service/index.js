...
app.get('/', (req, res) => res.json({ service: 'Baalvion Tenant / White-Label Service', version: config.apiVersion, surface: ['registry', 'branding', 'domains', 'entitlements'] }));
app.get('/health', (req, res) => res.json({
    status: 'ok', service: 'tenant-service', timestamp: new Date().toISOString(),
...
const start = async () => {
    try {
        ...
        logger.info('[tenant-service] DB ready');
    } catch (err) {
        logger.error({ err: err.message }, '[tenant-service] DB init failed');
        process.exit(1);
    }
