...
const listPublished = async (req, res, next) => {
    try { const rows = await db.Topic.findAll({ where: publicWhere, order: [['name', 'ASC']], limit: 5000 }); cache(res); return sendSuccess(req, res, rows); }
    catch (err) { return next(err); }
};
const getPublished = async (req, res, next) => {
    try {
        const row = await db.Topic.findOne({ where: { ...publicWhere, slug: req.params.slug } });
        if (!row) return next(new AppError('NOT_FOUND', 'Topic not found', 404));
...
const hiddenSlugs = async (req, res, next) => {
    try { const rows = await db.Topic.findAll({ where: { archived: true }, attributes: ['slug'] }); cache(res); return sendSuccess(req, res, rows.map((r) => r.slug)); }
    catch (err) { return next(err); }
};
