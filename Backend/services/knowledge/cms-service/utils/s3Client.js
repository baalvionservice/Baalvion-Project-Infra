 ...
 * crypto only). The `minio`/`@aws-sdk` packages can't be installed into this pnpm
 * workspace cleanly, so we sign requests ourselves. Path-style addressing,
 * region us-east-1 (MinIO default). Covers what the media library needs:
 * ensureBucket, putObject, deleteObject, and presigned GET URLs.
 */
