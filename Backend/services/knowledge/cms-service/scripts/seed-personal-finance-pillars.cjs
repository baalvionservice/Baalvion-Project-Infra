'use strict';
/*
 * Seed the "Personal Finance Pillars" content program into the Imperialpedia CMS
 * (cms-service) and publish. Sibling program to seed-investing-pillars.cjs, same
 * engine (cms-seed-lib.cjs), covering the 14 non-investing categories: Savings,
 * Credit Cards, Loans, Mortgages, Auto Loans, Student Loans, Indicators, Economy,
 * Inflation, GDP, Unemployment, Interest Rates, Fiscal Policy, Monetary Policy.
 * Each category ships 1 pillar page + 5 supporting cluster articles.
 *
 * All 14 categories are built (84 documents: 14 pillars + 70 supporting
 * articles), each following personal-finance-pillars-savings.data.cjs as the
 * template (categorySlug, categoryName, sources, pillar{...}, articles[5]).
 *
 * Content : ./personal-finance-pillars-<category>.data.cjs
 * Engine  : ./cms-seed-lib.cjs (converter + multi-category API runner)
 *
 * USAGE
 *   node scripts/seed-personal-finance-pillars.cjs --export          # build JSON, no creds
 *   CMS_TOKEN=<bearer> node scripts/seed-personal-finance-pillars.cjs --dry-run
 *   CMS_TOKEN=<bearer> node scripts/seed-personal-finance-pillars.cjs                # create + publish (skips existing)
 *   CMS_TOKEN=<bearer> node scripts/seed-personal-finance-pillars.cjs --update       # PATCH existing + create new
 *   ... --category=savings          # limit to one category
 *   ... --only=emergency-funds      # limit to one slug
 *
...
const CATEGORIES = [
  require('./personal-finance-pillars-savings.data.cjs'),
  require('./personal-finance-pillars-credit-cards.data.cjs'),
  require('./personal-finance-pillars-loans.data.cjs'),
  require('./personal-finance-pillars-mortgages.data.cjs'),
  require('./personal-finance-pillars-auto-loans.data.cjs'),
  require('./personal-finance-pillars-student-loans.data.cjs'),
  require('./personal-finance-pillars-indicators.data.cjs'),
  require('./personal-finance-pillars-economy.data.cjs'),
  require('./personal-finance-pillars-inflation.data.cjs'),
  require('./personal-finance-pillars-gdp.data.cjs'),
  require('./personal-finance-pillars-unemployment.data.cjs'),
  require('./personal-finance-pillars-interest-rates.data.cjs'),
  require('./personal-finance-pillars-fiscal-policy.data.cjs'),
  require('./personal-finance-pillars-monetary-policy.data.cjs'),
];
...
const PERSONAL_FINANCE_DISCLAIMER = 'This content is for educational purposes only and does not constitute financial, legal, or tax advice. Rates, terms, and account features vary by provider and change over time — confirm current details directly with the institution. Consider consulting a licensed financial advisor for guidance specific to your situation.';

function buildCta(categoryName) {
  return {
    newsletter: `Get weekly insights on ${categoryName.toLowerCase()} and smarter personal finance — subscribe to the ImperialPedia newsletter.`,
    disclaimer: PERSONAL_FINANCE_DISCLAIMER,
...
function buildDoc(item, category, isPillar) {
  ...
  return {
    ...
    customFields: {
      ...
      // Editorial roadmap: cluster-article ideas this page is meant to anchor.
      // Surfaced in the admin content-brief panel; not rendered on the public page.
      contentStrategy: {
...
async function main() {
  ...
  console.log('Personal Finance Pillars content seed');
  ...
  const runner = createRunner({
    ...
    outDir: process.env.OUT_DIR || path.join(process.env.TEMP || '/tmp', 'personal-finance-pillars-seed'),
  });
