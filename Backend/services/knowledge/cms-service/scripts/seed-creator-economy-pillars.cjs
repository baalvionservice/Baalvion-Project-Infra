'use strict';
/*
 * Seed the "Creator Economy" content program into the Imperialpedia CMS
 * (cms-service) and publish. Sibling program to seed-budgeting-pillars.cjs
 * and seed-investing-pillars.cjs, same engine (cms-seed-lib.cjs).
 *
 * Covers the Creator Economy pillar + 5 cluster articles:
 * 1. How Much Does YouTube Pay for 1,000 Views? (Pillar)
 * 2. YouTube RPM vs CPM Explained (Cluster)
 * 3. How to Qualify for YouTube Monetization (YPP Requirements) (Cluster)
 * 4. How Much Can a Website Earn from 100,000 Monthly Visitors? (Cluster)
 * 5. How Google AdSense Works (Cluster)
 * 6. How Instagram Pays Creators (and What It Doesn't) (Cluster)
 *
 * Engine  : ./cms-seed-lib.cjs (converter + multi-category API runner)
 * Data    : ./creator-economy-pillars.data.cjs
 *
 * USAGE:
 *   node scripts/seed-creator-economy-pillars.cjs --export
 *   CMS_TOKEN=<bearer> node scripts/seed-creator-economy-pillars.cjs --dry-run
 *   CMS_TOKEN=<bearer> node scripts/seed-creator-economy-pillars.cjs
 */

const path = require('path');
const { markdownToBlocks, wordCount, slugify, createRunner } = require('./cms-seed-lib.cjs');

const CATEGORY = require('./creator-economy-pillars.data.cjs');
...
const CREATOR_DISCLAIMER = 'This content is for educational purposes only and does not constitute financial, legal, or tax advice. Creator earnings vary significantly based on niche, audience geography, platform policies, and seasonality. Always verify current platform terms directly with official creator documentation.';

function buildCta(categoryName) {
  return {
    newsletter: `Get weekly creator economy insights and revenue strategies — subscribe to the ImperialPedia newsletter.`,
    disclaimer: CREATOR_DISCLAIMER,
    relatedReading: `Explore more guides in our ${categoryName} hub.`,
  };
}
...
function collectDocs() {
  ...
  for (const article of (CATEGORY.articles || [])) {
    docs.push(buildDoc(article, CATEGORY, false));
  }
...
async function main() {
  ...
  if (!docs.length) throw new Error(`No matching documents (only=${ONLY || 'none'})`);

  console.log('Creator Economy Hub Pillars content seed');
  console.log(`  target     : ${TARGET_BASE}`);
  console.log(`  site       : ${SITE}`);
  console.log(`  category   : ${CATEGORY.categorySlug}`);
  ...
  const runner = createRunner({
    ...
    outDir: process.env.OUT_DIR || path.join(process.env.TEMP || '/tmp', 'creator-economy-pillars-seed'),
  });
