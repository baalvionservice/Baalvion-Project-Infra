'use strict';
/*
 * Seed batch 2 of previously-empty imperialpedia category slugs — 12 categories
 * that already existed as real CmsCategory rows (some nested: personal-finance >
 * budgeting/insurance/taxes/income, markets > market-news/company-news, reviews >
 * bank-reviews/broker-reviews/credit-card-reviews/robo-advisors/insurance-reviews,
 * plus the top-level `reviews` hub itself) but had zero published content, so their
 * live pages rendered blank. Same content shape and seed engine as
 * seed-missing-categories-2026-07.cjs (pillar + 5-cluster-article per category).
 *
 * Content : 1 pillar + 5 cluster articles per category, in the sibling
 *           `*-pillars-*.data.cjs` files next to this script.
 * Engine  : ./cms-seed-lib.cjs (resolveCategoryId walks the nested category tree,
 *           so it correctly reuses each category's existing id instead of
 *           re-creating it — verified against the live tree before writing this).
 *
 * USAGE (same contract as seed-missing-categories-2026-07.cjs)
 *   node scripts/seed-missing-categories-2026-07-batch2.cjs --export
 *   CMS_TOKEN=<bearer> node scripts/seed-missing-categories-2026-07-batch2.cjs --dry-run
 *   CMS_TOKEN=<bearer> node scripts/seed-missing-categories-2026-07-batch2.cjs                # create + publish (skips existing)
 *   ... --category=budgeting|insurance|taxes|income|market-news|company-news|reviews|bank-reviews|broker-reviews|credit-card-reviews|robo-advisors|insurance-reviews
 *   ... --only=<slug>
 */
...
const CATEGORIES = [
  require('./personal-finance-pillars-budgeting.data.cjs'),
  require('./personal-finance-pillars-insurance.data.cjs'),
  require('./personal-finance-pillars-taxes.data.cjs'),
  require('./personal-finance-pillars-income.data.cjs'),
  require('./markets-pillars-market-news.data.cjs'),
  require('./markets-pillars-company-news.data.cjs'),
  require('./reviews-pillars-overview.data.cjs'),
  require('./reviews-pillars-bank-reviews.data.cjs'),
  require('./reviews-pillars-broker-reviews.data.cjs'),
  require('./reviews-pillars-credit-card-reviews.data.cjs'),
  require('./reviews-pillars-robo-advisors.data.cjs'),
  require('./reviews-pillars-insurance-reviews.data.cjs'),
];
...
async function main() {
  ...
  console.log('Missing Categories (2026-07) content seed — batch 2');
  ...
  const runner = createRunner({
    ...
    outDir: process.env.OUT_DIR || path.join(process.env.TEMP || '/tmp', 'missing-categories-2026-07-batch2-seed'),
  });
