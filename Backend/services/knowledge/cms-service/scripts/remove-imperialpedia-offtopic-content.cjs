'use strict';
/*
 * Deletes off-topic / internal-demo content records from the Imperialpedia CMS
 * website that have no place on a personal-finance encyclopedia:
 *   - the-baalvion-empire        (internal company-lore test content, category "History")
 *   - quantum-computing-overview (unrelated tech topic, ~4-word stub)
 *   - baalvion-unified-cms-pipeline (internal platform announcement, not reader-facing news)
 *
 * USAGE
 *   node scripts/remove-imperialpedia-offtopic-content.cjs --dry-run
 *   CMS_TOKEN=<bearer> node scripts/remove-imperialpedia-offtopic-content.cjs
 *
...
const SLUGS_TO_REMOVE = ['the-baalvion-empire', 'quantum-computing-overview', 'baalvion-unified-cms-pipeline'];
...
async function main() {
  console.log('Imperialpedia off-topic content removal');
