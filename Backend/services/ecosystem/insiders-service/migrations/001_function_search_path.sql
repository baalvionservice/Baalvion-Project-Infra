...
-- to the `insiders` schema (they run with the caller's path otherwise, e.g. when
-- invoked as insiders.fn() over a public-default connection).
SET search_path TO insiders, public;

CREATE OR REPLACE FUNCTION increment_thread_views(thread_id UUID)
RETURNS VOID LANGUAGE SQL SET search_path = insiders, public AS $$
  UPDATE forum_threads SET views = COALESCE(views, 0) + 1 WHERE id = thread_id
$$;

CREATE OR REPLACE FUNCTION has_role(_user_id UUID, _role app_role)
RETURNS BOOLEAN LANGUAGE SQL STABLE SET search_path = insiders, public AS $$
  SELECT EXISTS (SELECT 1 FROM user_roles WHERE user_id = _user_id AND role = _role)
$$;

CREATE OR REPLACE FUNCTION create_notification(
  p_user_id UUID, p_type TEXT, p_title TEXT, p_message TEXT, p_link TEXT DEFAULT NULL
) RETURNS UUID LANGUAGE plpgsql SET search_path = insiders, public AS $$
DECLARE v_id UUID;
BEGIN
