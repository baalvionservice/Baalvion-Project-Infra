...
// Indexing/admin: a trusted producer presents X-Internal-Key; else fall back to user auth.
const internalOrUser = (req, res, next) => {
    const key = req.headers['x-internal-key'];
    if (config.internalApiKey && key && key === config.internalApiKey) {
        req.internal = true;
        req.auth = req.auth || { userId: 'service', roles: ['service', 'super_admin'], permissions: [], orgId: null };
        return next();
    }
