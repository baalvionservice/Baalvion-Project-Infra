...
app.get('/', (req, res) => res.json({ service: 'Baalvion Developer Platform', version: config.apiVersion, surface: ['api-keys', 'webhooks', 'openapi-catalog', 'sandbox'] }));
app.get('/health', (req, res) => res.json({
    status: 'ok', service: 'developer-service', timestamp: new Date().toISOString(),
    rs256: jwt.isRs256Enabled(), redis: redis.isAvailable() ? 'ok' : 'unavailable',
    webhookDelivery: config.webhooks.deliveryEnabled ? 'on' : 'off',
    eventConsumer: config.eventBus.consume ? 'on' : 'off',
}));
...
const start = async () => {
    try {
        ...
        logger.info('[developer-service] DB ready');
    } catch (err) {
        logger.error({ err: err.message }, '[developer-service] DB init failed');
        process.exit(1);
    }
