'use strict';
// inventory-service Policy Enforcement Point — the SHARED @baalvion/commerce-rbac PEP. Inventory
// does NOT own the store table, so store→country is resolved from the RBAC tenant tree. RBAC is
// the sole authority; every inventory endpoint is store-scoped.
...
function getRedis() {
    if (!redis) {
        ...
        redis.on('error', (e) => console.error('[inventory-service rbac]', e.message));
    }
    return redis;
}
const cache = {
...
const audit = createAuditEmitter({ service: 'inventory-service', redis: getRedis() });
const scope = createScopeResolver({
    ...
    audit, keyPrefix: 'inventory',
});
const resolveStoreScope = createRbacStoreCountryResolver({ rbacClient, cache, ttl: config.cache.rbacScopeTtl, keyPrefix: 'inventory' });
