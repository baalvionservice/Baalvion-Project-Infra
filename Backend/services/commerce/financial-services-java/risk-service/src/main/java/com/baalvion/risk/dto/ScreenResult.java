...
/**
 * Strict external screening contract returned by {@code POST /api/v1/sanctions/screen}. Backward
 * compatible — original fields unchanged; {@code matches[].sourceConfidence} and {@code sourcesChecked}
 * are additive:
 * <pre>{ status, confidence, matches: [{ name, source, program?, sourceConfidence }], sourcesChecked: [...] }</pre>
...
public class ScreenResult {
  ...
  /** Jurisdictions screened against this request (e.g. ["EU","OFAC","UN"]). Additive. */
  ...
  public static ScreenResult from(ScreeningResponse r) {
    List<ScreenMatch> matches = r.getHits() == null ? List.of()
      : r.getHits().stream()
          .map(h -> ScreenMatch.builder()
            ...
            // Prefer the short jurisdiction code (OFAC|EU|UN); fall back to the internal source.
            .source(h.getSource() != null ? h.getSource() : h.getListSource())
            .program(h.getPrograms() == null || h.getPrograms().isEmpty()
              ? null : String.join(", ", h.getPrograms()))
            .sourceConfidence(h.getSourceConfidence())
            .build())
          .toList();
    ...
    return ScreenResult.builder()
      ...
      .matches(matches)
      .sourcesChecked(r.getSourcesChecked() == null ? List.of() : r.getSourcesChecked())
      .build();
  }
}
