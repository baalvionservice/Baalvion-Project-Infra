'use strict';
/**
 * Shipment Workflow — webhook fan-out (War Room 4, Prompt 2).
 *
 * Enqueues one job per (transition × subscription) onto the BullMQ
 * `workflow_webhook` queue, which already provides bounded retries, exponential
 * backoff and a dead-letter queue (queue/index.js + queue/workers.js). The
 * matching `workflow_webhook` processor performs the signed HTTPS POST and
 * advances the persisted delivery row (pending → delivered / failed / dead).
 *
 * Queue enqueue is BEST-EFFORT and post-commit: if Redis is unavailable the
 * delivery row stays `pending` and is recoverable by replay/sweep — the
 * transaction that changed state has already committed durably.
 */
...
let enqueueFn = null;
function getEnqueue() {
    if (enqueueFn) return enqueueFn;
    try {
        // Lazy: queue/index.js opens a Redis connection on import — only do it
        // when a webhook actually needs delivering.
        enqueueFn = require('../../queue').enqueue;
    } catch {
        enqueueFn = null;
    }
    return enqueueFn;
...
const WEBHOOK_QUEUE = 'workflow_webhook';

/** HMAC-SHA256 over the exact JSON bytes, matching the worker's verification. */
function sign(secret, body) {
    // No hardcoded fallback: a publicly-known key ('baalvion') would let anyone forge
    // a signature the receiver accepts. The subscription secret is mandatory.
    ...
    for (const d of deliveries) {
        try {
            ...
            await enqueue(WEBHOOK_QUEUE, 'deliver', {
                ...
                secret: d.secret,
                payload: d.payload,
            }, { jobId: `wfwh:${d.deliveryId}` });
            enqueued += 1;
        } catch {
            /* leave the row pending for replay; never throw into the request path */
        }
    }
    return { enqueued };
}
 ...
 * BullMQ retries; on the final attempt the row is marked failed/dead.
 */
async function processDelivery(job) {
    ...
    const delivery = deliveryId ? await db.WorkflowWebhookDelivery.findByPk(deliveryId) : null;
        ...
        try {
            res = await fetch(url, {
                ...
                headers: {
                    ...
                    'X-Baalvion-Event': (payload && payload.type) || 'shipment_workflow.event',
