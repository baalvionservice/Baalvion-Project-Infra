...
const start = async () => {
    ...
    // Graceful shutdown: drain HTTP (also closes the bound WS server), stop BullMQ
    // workers (closes their Redis connections), then close the DB pool.
    registerShutdown('queue-workers', async () => {
        if (process.env.QUEUE_WORKERS !== 'false') {
