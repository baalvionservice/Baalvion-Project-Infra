 ...
 * This app has no email provider credentials of its own — see lib/email.ts's TODO, which this
 * replaces as the real transport for contact/partnership submissions.
 *
...
export async function sendLead(
  ...
  try {
    const res = await fetch(LEAD_API_URL, {
      ...
      // Never let a slow/unreachable backend hang the submitter's request indefinitely.
      signal: AbortSignal.timeout(8000),
    });

    if (!res.ok) {
      const body = await res.text().catch(() => "");
      serverLogger.error("leadNotify.upstream_error", { site, status: res.status, body: body.slice(0, 300) });
      return { success: false, error: `upstream_${res.status}` };
    }

    return { success: true };
  } catch (error: unknown) {
    serverLogger.error("leadNotify.request_failed", {
      site,
