 ...
 *
 * `?next=` is attacker-controllable and reaches `window.location.href` after login.
 * A bare `startsWith('/')` check is not enough: `//evil.com` and `/\evil.com` both
 * pass it and resolve off-origin. This validator only honors same-origin,
 * root-relative paths; anything that could escape the current origin collapses to
 * the fallback.
 */
