...
/**
 * Dedicated Debt Management hub — follows the same rendering pattern proven by
 * CreditHub/BudgetingHub/BankingHub (long-form keyTakeaways + named sections
 * sourced from topic-config.ts, single CMS-first article feed, a real "Explore
 * Debt Guides" index of the category's own specialist articles, aggregated
 * FAQ). Replaces the generic CategoryFeed this route used previously.
 */
export async function DebtHub() {
  ...
  // "Explore Debt Guides" — every remaining real specialist article in this
  // category that isn't already shown above, so the full guide library is
  // reachable from the pillar without hardcoding article slugs/URLs.
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ Debt Guides &middot; Reviewed &amp; Updated Regularly
      </p>

      {/* Pillar primer — same keyTakeaways/sections pattern the other flagship
          hubs (banking, budgeting, credit, stocks) render, so /debt is a real
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        {/* Explore Debt Guides — every real specialist article in this category */}
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          <h3 className="text-lg font-bold text-foreground">Stay on Top of Your Debt</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical debt and money guidance delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>

        {/* Explore More — internal cross-links to related pillars */}
        <section>
          <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-6 pb-2">
            Explore More
          </h3>
