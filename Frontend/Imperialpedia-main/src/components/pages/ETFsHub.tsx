 ...
 *  ETFs has no CMS sub-categories of its own. */
const EXPLORE_INVESTING: Array<{
  ...
  { slug: "mutual-funds", label: "Mutual Funds", description: "How daily-priced fund shares differ from an ETF's intraday trading", icon: PiggyBank },
  { slug: "stocks", label: "Stocks", description: "The individual securities many equity ETFs are built from", icon: TrendingUp },
  { slug: "bonds", label: "Bonds", description: "How a bond ETF packages many bonds into one tradable fund", icon: Landmark },
  { slug: "commodities", label: "Commodities", description: "How a commodity ETF offers exposure without the physical asset", icon: Wheat },
  { slug: "portfolio", label: "Portfolio Management", description: "Building a diversified allocation with a handful of low-cost funds", icon: Briefcase },
];
...
/**
 * Dedicated ETFs hub — the ETFs educational pillar, following the same
...
export async function ETFsHub() {
  ...
  if (!faqSource.some((a) => a.faq?.length)) {
    faqSource = staticArticleList().filter((a) => a.category === "ETFs");
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ ETF Guides &middot; Reviewed &amp; Updated Regularly
      </p>
      ...
      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          ...
          /etfs is a real educational resource in its own right. */}
