...
const CDRATES_FAQS: FaqItem[] = [
  ...
  { question: "How does CdRates work?", answer: "CdRates works by providing features such as ... (brief description).", link: { label: "Learn more about CdRates", href: "/cdrates" } },
  { question: "Who should consider CdRates?", answer: "Anyone looking to improve their cdrates situation can benefit.", link: { label: "Learn more about CdRates", href: "/cdrates" } },
  { question: "What are common fees for CdRates?", answer: "Typical fees include ... and can often be avoided with ...", link: { label: "Learn more about CdRates", href: "/cdrates" } },
  { question: "How to compare CdRates options?", answer: "Look at interest rates, fees, features, and user reviews to decide.", link: { label: "Learn more about CdRates", href: "/cdrates" } },
];

const CDRATES_PRODUCT_TOPICS: Array<{ slug: string; label: string; icon: React.ComponentType<{ className?: string }> }> = [
  { slug: "cdrates", label: "CdRates Overview", icon: Star },
 ...
 *  of "cd-rates" itself. */
const EXPLORE_CDS: Array<{
  slug: string;
  label: string;
  description: string;
  icon: React.ComponentType<{ className?: string }>;
}> = [
  { slug: "savings", label: "Savings Accounts", description: "Why a savings account still matters for money you might need sooner", icon: PiggyBank },
  { slug: "money-market", label: "Money Market Accounts", description: "A more liquid alternative that still beats standard savings rates", icon: Landmark },
  { slug: "checking", label: "Checking Accounts", description: "How everyday transaction accounts differ from CDs", icon: Wallet },
  { slug: "interest-rates", label: "Interest Rates", description: "How broader rate conditions shape what CDs pay", icon: Percent },
  { slug: "banking-reviews", label: "Banking Reviews", description: "Independent comparisons of current CD rates by bank", icon: Star },
];
...
/**
 * Dedicated CD Rates hub — following the same rendering pattern as
 ...
 * previously used. The URL is /cd-rates, but the content explains CDs
 * broadly rather than merely listing rates (no verified current rate data
 * exists in this repo to publish).
 */
export async function CdRatesHub() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ CD Guides &middot; Reviewed &amp; Updated Regularly
      </p>

      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          other flagship hubs (banking, credit, bonds) render, so /cd-rates is a
          real educational resource in its own right, not just a rate table. */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        {/* Explore CDs & Banking — real sibling pages this pillar links down to */}
        <section>
          <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-gray-400 mb-6 pb-2">
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          <h3 className="text-lg font-bold text-foreground">Grow Savings Safely</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical CD and savings guidance delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>
