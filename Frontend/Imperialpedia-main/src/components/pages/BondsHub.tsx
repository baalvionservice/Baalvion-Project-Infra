 ...
 *  bonds has no CMS sub-categories of its own. */
const EXPLORE_INVESTING: Array<{
  ...
  { slug: "stocks", label: "Stocks", description: "How equity investing works before comparing it to fixed income", icon: TrendingUp },
  { slug: "etfs", label: "ETFs", description: "How a bond ETF packages many bonds into one tradable fund", icon: PieChart },
  { slug: "mutual-funds", label: "Mutual Funds", description: "Actively managed bond funds priced once per day at NAV", icon: PiggyBank },
  { slug: "portfolio", label: "Portfolio Management", description: "How bonds fit alongside stocks in a diversified allocation", icon: Briefcase },
  { slug: "retirement", label: "Retirement Investing", description: "Why fixed income allocations often grow closer to retirement", icon: Umbrella },
];
...
/**
 * Dedicated Bonds hub — the Bonds & Fixed Income educational pillar, following
 ...
 * aggregated FAQ, sibling-pillar nav cards) rather than the generic CategoryFeed
 * this route previously used.
 */
export async function BondsHub() {
  ...
  if (!faqSource.some((a) => a.faq?.length)) {
    faqSource = staticArticleList().filter((a) => a.category === "Bonds");
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ Bond &amp; Fixed Income Guides &middot; Reviewed &amp; Updated Regularly
      </p>
      ...
      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          ...
          /bonds is a real educational resource in its own right. */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          ...
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical fixed-income guidance delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>
