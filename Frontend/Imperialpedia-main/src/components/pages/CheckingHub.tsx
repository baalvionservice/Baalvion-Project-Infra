 ...
 *  of "checking" itself. */
const EXPLORE_CHECKING: Array<{
  slug: string;
  label: string;
  description: string;
  icon: React.ComponentType<{ className?: string }>;
}> = [
  { slug: "savings", label: "Savings Accounts", description: "How to split day-to-day spending money from savings", icon: PiggyBank },
  { slug: "money-market", label: "Money Market Accounts", description: "A higher-yield alternative for balances you don't need daily", icon: Landmark },
  { slug: "cd-rates", label: "CD Rates", description: "Locking in a fixed rate for money you won't need for a while", icon: Calendar },
  { slug: "credit-cards", label: "Credit Cards", description: "How a linked credit card's grace period differs from a debit card's", icon: CreditCard },
  { slug: "banking-reviews", label: "Banking Reviews", description: "Independent comparisons of bank fees and features", icon: Star },
];
...
/**
 * Dedicated Checking hub — following the same rendering pattern as
 ...
 * previously used.
 */
export async function CheckingHub() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ Checking Account Guides &middot; Reviewed &amp; Updated Regularly
      </p>

      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          other flagship hubs (banking, credit, bonds) render, so /checking is a
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        {/* Explore Checking & Banking — real sibling pages this pillar links down to */}
        <section>
          <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-gray-400 mb-6 pb-2">
            <Wallet className="h-4 w-4" />
            Explore Checking &amp; Banking
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {EXPLORE_CHECKING.map((topic) => (
              <Link
                key={topic.slug}
                href={`/${topic.slug}`}
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          <h3 className="text-lg font-bold text-foreground">Bank Smarter, Every Day</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical checking and banking guidance delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>
