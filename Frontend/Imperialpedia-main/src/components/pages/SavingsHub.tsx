 ...
 *  of "savings" itself. */
const EXPLORE_SAVINGS: Array<{
  slug: string;
  label: string;
  description: string;
  icon: React.ComponentType<{ className?: string }>;
}> = [
  { slug: "checking", label: "Checking Accounts", description: "How everyday transaction accounts differ from savings", icon: Wallet },
  { slug: "cd-rates", label: "CD Rates", description: "Trading liquidity for a fixed rate over a fixed term", icon: Calendar },
  { slug: "money-market", label: "Money Market Accounts", description: "A savings alternative with check-writing or debit access", icon: Landmark },
  { slug: "emergency-fund", label: "Emergency Fund", description: "How much to keep set aside and where to hold it", icon: Umbrella },
  { slug: "budgeting", label: "Budgeting", description: "Working savings goals into a broader monthly budget", icon: PiggyBank },
];
...
/**
 * Dedicated Savings hub — following the same rendering pattern as
 ...
 * previously used.
 */
export async function SavingsHub() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ Savings Guides &middot; Reviewed &amp; Updated Regularly
      </p>

      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          other flagship hubs (banking, credit, bonds) render, so /savings is a
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        {/* Explore Savings — real sibling pages this pillar links down to */}
        <section>
          <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-gray-400 mb-6 pb-2">
            <PiggyBank className="h-4 w-4" />
            Explore Savings &amp; Banking
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {EXPLORE_SAVINGS.map((topic) => (
              <Link
                key={topic.slug}
                href={`/${topic.slug}`}
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          <h3 className="text-lg font-bold text-foreground">Build Your Savings, Smarter</h3>
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical savings guidance delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>
