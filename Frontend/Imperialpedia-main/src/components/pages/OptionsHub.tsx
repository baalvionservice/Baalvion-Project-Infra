 ...
 *  options has no CMS sub-categories of its own. */
const EXPLORE_INVESTING: Array<{
  ...
  { slug: "stocks", label: "Stocks", description: "The underlying shares most equity options are written on", icon: TrendingUp },
  { slug: "etfs", label: "ETFs", description: "How ETF options work similarly to single-stock options", icon: PieChart },
  { slug: "portfolio", label: "Portfolio Management", description: "How hedging fits into broader portfolio risk management", icon: Briefcase },
  { slug: "brokers", label: "Brokers", description: "What to check before a brokerage account can trade options", icon: Building2 },
];
...
/**
 * Dedicated Options hub — the Options educational pillar, following the same
 * rendering pattern as CreditHub/StocksHub (long-form keyTakeaways + named
 * sections sourced from topic-config.ts, single CMS-first article feed,
 * aggregated FAQ, sibling-pillar nav cards) rather than the generic
 * CategoryFeed this route previously used. Content is deliberately cautious
 * and educational-only — see topic-config.ts's `options` entry — options are
 * never framed as an easy or reliable way to make money.
 */
export async function OptionsHub() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground -mt-2 pb-6">
        {Math.max(articles.length, 10)}+ Options Guides &middot; Reviewed &amp; Updated Regularly
      </p>
      ...
      {/* Pillar primer — same keyTakeaways/sections pattern CategoryFeed and the
          ...
          /options is a real educational resource in its own right. */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 space-y-16">
        ...
        <section className="flex flex-col items-center gap-4 rounded-2xl bg-gray-50 py-12 text-center">
          ...
          <p className="max-w-md text-sm text-muted-foreground">
            Get practical, risk-aware market education delivered weekly — no guaranteed outcomes, just clear explanations.
          </p>
          <NewsletterForm />
        </section>
