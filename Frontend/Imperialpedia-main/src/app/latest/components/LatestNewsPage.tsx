...
/**
 * Live-CMS-first "Latest News" feed — mirrors the fallback chain used across
 * the site (getPublishedNews → bundled demo newsArticles) so the page is
 * never empty and every card links to a real, resolvable article URL.
 */
export async function LatestNewsPage() {
  const liveNews = await getPublishedNews(60);
  const articles: NewsArticle[] = liveNews.length > 0 ? liveNews : newsArticles;
    ...
    <div className="min-h-screen bg-white mt-12 w-full overflow-x-hidden">
      ...
      <div className="max-w-3xl mx-auto px-2 sm:px-3 lg:px-4 pb-8 sm:pb-12 w-full">
        ...
        <div className="mt-6 pt-4 border-t-[3px] border-yellow-400">
          <h3 className="text-base sm:text-[15px] font-bold tracking-[1px] uppercase text-[#0a2756] mb-4 px-0.5">
            Trending Now
          </h3>
