...
// Free equivalents for the World vertical's Imperialpedia-style type system:
// Proxima Nova/Averta (headlines, UI) -> Inter, FoundersGroteskCond
// (ticker/kicker labels) -> Barlow Condensed, Lato (body) -> Lato (Imperialpedia's
// actual body font, and free). Scoped via CSS vars so the sitewide
// Imperialpedia serif theme is untouched outside /world/*.
...
/**
 * World news vertical shell — its own Imperialpedia-style masthead and footer,
 * intentionally distinct from the sitewide Imperialpedia-style Navbar/Footer
...
export default function WorldLayout({ children }: { children: ReactNode }) {
  return (
    <div
      className={`world-shell imperialpedia-shell flex min-h-screen flex-col bg-black ${worldSans.variable} ${worldCondensed.variable} ${worldBody.variable}`}
    >
      <TopNav />
