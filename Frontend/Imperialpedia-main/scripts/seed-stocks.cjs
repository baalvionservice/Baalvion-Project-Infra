...
'use strict';
/**
 * Seed Imperialpedia's "Stocks" pillar content into the central Baalvion CMS so it is
 ...
 * scripts/seed-investing.cjs, pointed at the `stocks` category instead.
 *
 * It (idempotently):
 *   - logs in as a CMS publisher (super_admin bypasses workflow role levels)
 *   - resolves the `imperialpedia` website
 *   - ensures the `stocks` category exists
 *   - reads every article spec in content/stocks/articles/*.json → contentType 'article'
...
const CATEGORY = { name: 'Stocks', slug: 'stocks' };

const ROOT = path.resolve(__dirname, '..', 'content', 'stocks');
...
async function main() {
  ...
  // Length guardrail — this pack targets ~550-900 words per article.
  for (const s of articleSpecs) {
    const wc = wordCount(s);
    if (wc < 550) console.warn(`  ! SHORT: ${s.slug} ~${wc} words (< 550 minimum)`);
  }
  ...
  if (DRY) {
    ...
    for (const s of articleSpecs) {
      try {
        ...
        if (doc.customFields.wordCount < 550) issues.push(`short:${doc.customFields.wordCount}`);
  ...
  console.log(JSON.stringify({
    ...
    public: `${CMS}/public/${WEBSITE_SLUG}/content?contentType=article&categorySlug=stocks&limit=100`,
    failures: failed,
  }, null, 2));
