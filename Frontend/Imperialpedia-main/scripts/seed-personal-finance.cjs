...
async function main() {
  ...
  if (DRY) {
    ...
    const check = (specs, contentType, appendAuthor) => {
      for (const s of specs) {
        try {
          const doc = buildContentDoc(s, { contentType, categoryId: 'DRY', appendAuthor });
          const issues = [];
  ...
  const docs = [
    ...articleSpecs.map((s) => buildContentDoc(s, { contentType: 'article', categoryId, appendAuthor: true })),
    ...pageSpecs.map((s) => buildContentDoc(s, { contentType: 'page', categoryId: null, appendAuthor: false })),
  ];
