...
'use strict';
/**
 * Seed Imperialpedia's "Investing" pillar content into the central Baalvion CMS so it is
 * fully manageable from the admin console (admin-platform :3030) AND served to the live
 * site (:3029) through the public delivery API — the same pipeline as
 * scripts/seed-personal-finance.cjs, pointed at the `investing` category instead.
 *
 * It (idempotently):
 *   - logs in as a CMS publisher (super_admin bypasses workflow role levels)
 *   - resolves the `imperialpedia` website
 *   - ensures the `investing` category exists
 *   - reads every article spec in content/investing/articles/*.json → contentType 'article'
 ...
 *
 * IMPORTANT: this writes real content into whatever CMS_URL points at. Defaults below
 * target localhost — pointing this at a production cms-service/admin.baalvion.com is a
 * deliberate, real action and should only be run intentionally, not by default.
 *
 *   AUTH_URL=http://localhost:3001/v1/auth \
 *   CMS_URL=http://localhost:3011/api/v1 \
 *   SUPERADMIN_EMAIL=superadmin@baalvion.com \
 *   SUPERADMIN_PASSWORD=*** \
 *   node scripts/seed-investing.cjs
 *
 * Flags:
...
const CATEGORY = { name: 'Investing', slug: 'investing' };

const ROOT = path.resolve(__dirname, '..', 'content', 'investing');
const ARTICLES_DIR = path.join(ROOT, 'articles');

const DRY = process.argv.includes('--dry');

// Shared spec -> contentBlocks builder (single source of truth for seed + static snapshot).
const { buildContentDoc, wordCount } = require('./lib/content-builder.cjs');
...
// ── main ──────────────────────────────────────────────────────────────────────
async function main() {
  const articleSpecs = readSpecs(ARTICLES_DIR);
  console.log(`Specs found: ${articleSpecs.length} articles`);

  // Length guardrail — this pack targets ~700 words per article, not the 1500+ used
  // by the personal-finance pack, so the floor here is lower.
  for (const s of articleSpecs) {
    const wc = wordCount(s);
    if (wc < 500) console.warn(`  ! SHORT: ${s.slug} ~${wc} words (< 500 minimum)`);
  }
  ...
  if (DRY) {
    ...
    for (const s of articleSpecs) {
      try {
        const doc = buildContentDoc(s, { contentType: 'article', categoryId: 'DRY', appendAuthor: true });
        const issues = [];
        ...
        if (doc.customFields.wordCount < 500) issues.push(`short:${doc.customFields.wordCount}`);
        if (issues.length) problems++;
        rows.push(`  ${issues.length ? 'x' : 'OK'} ${doc.slug.padEnd(45)} ${String(doc.customFields.wordCount).padStart(4)}w ${String(doc.contentBlocks.length).padStart(3)}blk ${issues.join(',')}`);
      } catch (e) {
        problems++;
        rows.push(`  x ${s.slug} BUILD ERROR: ${e.message}`);
      }
    }
    console.log(`DRY — validating ${articleSpecs.length} docs:\n${rows.join('\n')}`);
  ...
  const docs = articleSpecs.map((s) => buildContentDoc(s, { contentType: 'article', categoryId, appendAuthor: true }));
  ...
  console.log(JSON.stringify({
    ...
    public: `${CMS}/public/${WEBSITE_SLUG}/content?contentType=article&categorySlug=investing&limit=30`,
    failures: failed,
  }, null, 2));
