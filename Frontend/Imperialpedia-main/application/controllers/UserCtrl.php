...
// New Comment — removed old login-required version; public version is at the bottom of this file
...
public function forgot_password(){ 
    ...
    // Process new reader comment submission
    public function new_comment(){
        $this->load->database();
        $name = $this->input->post('commenter_name', TRUE);
        $comment = $this->input->post('comment_txt', TRUE);
        $page = $this->input->post('comment_page', TRUE);
        $redirect = $this->input->post('redirect_url', TRUE);

        if(!empty($name) && !empty($comment) && !empty($page)){
            $data = array(
                'commenter_name' => substr($name, 0, 250),
                'comment_userid' => $this->session->userdata('userid') ? $this->session->userdata('userid') : 'guest',
                'comment_page'   => $page,
                'comment_txt'    => $comment,
                'comment_approve'=> 'yes'
            );
            $this->db->insert('comment', $data);
            $this->session->set_flashdata('comment_success', 'Thank you! Your comment has been published successfully.');
        }
        if(!empty($redirect)){
            redirect($redirect);
        } else {
