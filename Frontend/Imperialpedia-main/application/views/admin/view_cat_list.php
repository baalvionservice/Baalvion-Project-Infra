<!-- Content Wrapper. Contains page content -->

<div class="content-wrapper">

   <section class="content-header">

      <h1> Category </h1>

      <ol class="breadcrumb">

         <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>

         <li class="active">Cats</li>

      </ol>

   </section>

   <?php $flash = $this->session->flashdata('msg'); if(!empty($flash)): ?>
   <div class="alert <?php echo (stripos($flash, 'fail') !== false || stripos($flash, 'but ') !== false) ? 'alert-warning' : 'alert-success'; ?>"><?php echo $flash; ?></div>
   <?php endif; ?>

   <a href="<?php echo base_url(); ?>imp-admin/add_cat" style="float:right;">Add Category</a>

   <br/>

   <table id="example" class="table table-striped table-bordered" style="width:100%;padding-left:1%;">

      <thead>

         <tr>

            <th>Category</th>

            <th>Posted time</th>

            <th>Action</th>

         </tr>

      </thead>

      <tbody>

         <?php 

            foreach($res as $val) { 

            ?>

         <tr>

            <td><?php echo $val['cat_name']; ?></td>

            <td><?php echo $val['added_date']; ?></td>

            <td><a href="<?php echo base_url(); ?>imp-admin/cat_edit/<?php echo $val['cat_id']; ?>">Edit</a>/<a onclick="del_confirm('<?php echo $val['cat_id']; ?>');" href="<?php echo base_url(); ?>imp-admin/del_cat/<?php echo $val['cat_id']; ?>">Delete</a></td>

         </tr>

         <?php } ?>

      </tbody>

   </table>

</div>





<!-- delete conformation  -->

<script>

   function del_confirm(val){

       var con = confirm('Are you sure want to delete this Category?'); 

       if(con==true){ 

           window.location.href="<?php echo base_url()?>imp-admin/del_cat/"+val;

       }else{ 

           return false; 

       }  

   }

</script>