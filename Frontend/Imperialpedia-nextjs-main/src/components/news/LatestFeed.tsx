          ...
          <TabsList className="w-full justify-start bg-transparent p-0 h-auto flex-wrap gap-2">
            {MAIN_TABS.map((t) => (
              <TabsTrigger
                key={t.value}
                value={t.value}
                className="border border-border bg-card px-4 py-2 text-xs font-semibold uppercase tracking-wider text-foreground shadow-none data-[state=active]:border-[hsl(var(--imperialpedia-red))] data-[state=active]:bg-[hsl(var(--imperialpedia-red))] data-[state=active]:text-white"
              >
                {t.label}
              </TabsTrigger>
            ))}
