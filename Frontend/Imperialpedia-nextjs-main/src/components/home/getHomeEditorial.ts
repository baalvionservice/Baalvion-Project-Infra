...
// "Personal Finance" is a retired category (410, see middleware.ts) with no
// live hub or articles — excluded from the generic topicGroups candidate
// pool so a "Personal Finance" section can never form and link readers into
// a dead end. "Imperialpedia Talks" is a video-teaser column (see
// ImperialpediaTalks.tsx) — excluded from the whole generic pool so a Talks
// piece never surfaces as a plain text lead/related/other-top-story link
// without its video treatment.
...
// hub every published category already gets) and finally to the homepage.
...
function topicHref(categoryName: string, categorySlug?: string): string {
  ...
  // /financial-intelligence (the previous fallback) is itself retired — see
  // middleware.ts. There's no longer a generic "everything else" article
  // namespace to fall back to, so this points at the homepage instead.
  return "/";
}
