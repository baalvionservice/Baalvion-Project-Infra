/**
 * For-Investors-and-Founders → auth-gateway (BFF) unified session, via @baalvion/auth-sdk.
 * Vite app — uses import.meta.env (not process.env / NEXT_PUBLIC).
 *
 * ADDITIVE + gated by VITE_BFF_MODE (default off). This is a Supabase auth ISLAND; this module does
 * NOT touch the Supabase client. At cutover it replaces Supabase login with the gateway (no token in
 * JS, identity from /auth/me, data via authFetch) — gated on island users existing in the canonical
 * auth store. Canonical roles[]/permissions[] returned as-is.
 */
