...
import {
  ProductBadge,
  getProductBadge,
} from '@/components/ui/ProductBadge';
...
export const ProductCard = memo(({ product }: ProductCardProps) => {
  ...
  const targetFlow = product.isVip
    ? 'private-order'
    : 'product';
  ...
  return (
    <article className="group relative flex flex-col bg-transparent overflow-hidden h-full luxury-reveal">
      <Link
        href={`/${countryCode}/${targetFlow}/${product.id}`}
        className="block relative aspect-[3/4] overflow-hidden bg-[#fcfcfc] border border-gray-50 shadow-sm"
        aria-label={`Registry Details for ${product.name}`}
      >
        <PlaceholderImage
          id={
            product.isVip
              ? 'home-mastery'
              : 'product-luxury-default'
          }
          alt={product.name}
        ...
        <div className="hidden lg:flex absolute inset-x-0 bottom-0 p-10 flex-col space-y-6 translate-y-full group-hover:translate-y-0 transition-transform duration-700 ease-in-out bg-white/98 backdrop-blur-3xl z-20 border-t border-gray-100 shadow-[0_-20px_50px_rgba(0,0,0,0.05)]">
