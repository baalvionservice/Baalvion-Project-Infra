...
export interface Product {
  ...
  // ── Resale / consignment provenance (commerce-service product detail, camelCase) ──
  // Surfaced on the product detail page: condition grade badge, authenticity guarantee,
  // one-of-a-kind flag, and a verifiable certificate-of-authenticity code.
...
// These mirror the real backend resale/consignment contracts surfaced on the
// storefront. They are intentionally separate from the admin/mock `Appointment`
// above (which the admin simulator store still uses).

/** Condition grade shared by consignment intake and product provenance. */
...
/** One item in a consignment request (and as returned on the created request). */
...
/** A consignment request as returned by the backend (POST /requests, GET /requests/mine). */
...
/** A single wishlist entry (wishlist-service, store-scoped). */
...
/** Stored wishlist for the authenticated shopper (GET /wishlists/.../mine). */
...
export type StoreAppointmentStatus =
  | "requested"
  | "confirmed"
  | "completed"
  | "cancelled"
...
/** A booked storefront appointment (order-service, store-scoped). Distinct from the
 *  admin/mock `Appointment` type above. */
...
/** Public certificate-of-authenticity verification result (GET /certificates/:code/verify). */
