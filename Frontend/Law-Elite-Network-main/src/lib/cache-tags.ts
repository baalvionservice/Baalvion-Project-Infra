...
/**
 * Every cached read of CMS / law-service content carries this tag, so one
 * revalidateTag() in the publish webhook drops all of them at once — including
 * pages the webhook's caller has no way to name (category hubs whose feed just
 * changed, author pages, the homepage rails).
 *
 * This — not the fetch-level revalidate window — is what makes an edit appear
 * on the site. The windows below it are the safety net for when the webhook
 * never fires.
 */
export const CONTENT_CACHE_TAG = 'law-content';
