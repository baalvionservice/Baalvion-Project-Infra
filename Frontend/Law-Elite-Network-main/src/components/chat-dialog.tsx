...
export function ChatDialog({ lawyerUid, lawyerName }: ChatDialogProps) {
  ...
  const handleStartChat = async () => {
    ...
    const existing = await repo.findExistingConversation(user.userId, lawyerUid);
    const convo = existing ?? (await repo.createConversation({ clientUid: user.userId, lawyerUid, role }));
    setConversation(convo);
    setIsConnecting(false);
  };

  // Message sync over REST (polling). Realtime moves to the WS gateway in a later phase.
  useEffect(() => {
    if (!conversation) {
      setMessages([]);
      return;
    }
    const convoId = conversation.conversationId ?? conversation.id;
    let active = true;
    const load = () => repo.getMessages(convoId).then((m: any[]) => { if (active) setMessages(m); });
    load();
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setInputText('');
    const convoId = conversation.conversationId ?? conversation.id;
    const sent = await repo.sendMessage(convoId, user.userId, text);
        ...
        <DialogHeader className="p-6 border-b border-white/5 bg-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-accent font-bold uppercase tracking-widest mt-0.5">Secure Channel</p>
            </div>
          </div>
        </DialogHeader>
          ...
          {isConnecting ? (
            ...
            <ScrollArea className="h-full p-6">
              <div className="space-y-4">
                {messages?.map((msg, i) => (
                  <div key={msg.messageId || msg.id || i} className={`flex ${msg.senderUid === user?.userId ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                      msg.senderUid === user?.userId
