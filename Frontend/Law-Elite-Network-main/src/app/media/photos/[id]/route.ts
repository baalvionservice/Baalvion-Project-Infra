...
/**
 * Serves photos held in law-service (any entity) from LEN's own origin, so the
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const upstream = await fetch(`${BASE_URL}/photos/${id}`, { next: { revalidate: 86400 } }).catch(() => null);
