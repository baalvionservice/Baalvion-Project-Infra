...
export async function generateStaticParams() {
  return (await getAllMedia('interview')).map((m) => ({ slug: m.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const item = await getMediaBySlug('interview', (await params).slug);
  if (!item) return { robots: { index: false } };
  return {
    title: item.title,
    description: `${item.title}: interview featuring ${item.subject.name}.`,
    alternates: { canonical: `${SITE}/interviews/${item.slug}` },
