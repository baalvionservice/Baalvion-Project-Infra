'use client';
/**
 * Phase 6E-3 — company-dashboard → auth-gateway (BFF) cookie client. ADDITIVE + TOGGLEABLE.
 *
 * Enabled ONLY when NEXT_PUBLIC_BFF_MODE === 'on'. When off (default), the existing
 * dashboard-service Bearer/localStorage path is used unchanged (rollback — NOT removed).
 ...
 *   - login     → if BFF_ENABLED, gwLogin()
 *   - bootstrap → if BFF_ENABLED, user = await gwMe()  (single source of truth, no LS)
 *   - data      → if BFF_ENABLED, gwApi('/dashboard/...') instead of axios+Bearer
