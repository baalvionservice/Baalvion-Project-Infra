/**
 * Proxy-BaalvionStack → auth-gateway (BFF) unified session, via @baalvion/auth-sdk.
 * Vite app — uses import.meta.env (not process.env / NEXT_PUBLIC).
 *
 * ADDITIVE + gated by VITE_BFF_MODE (default off); the app's existing auth path is left untouched
 * for rollback. At cutover this replaces it: no token in JS, identity from /auth/me, data via
 * authFetch (cookie + CSRF + single-flight refresh). Canonical roles[]/permissions[] are returned
 * as-is; mapping to any app-specific role vocabulary is RBAC-phase work.
 */
