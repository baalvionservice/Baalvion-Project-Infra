...
import { courtsApi, COURT_LEVELS, rowsOf, type CourtRecord } from '@/lib/law/legal';

const SELECT = 'h-9 rounded-md border border-input bg-background px-3 text-sm';
const PAGE = 50;

export default function LawCourtsPage() {
  ...
  useEffect(() => { setBreadcrumbs([{ label: 'Law Elite', href: '/law' }, { label: 'Courts' }]); }, [setBreadcrumbs]);
  useEffect(() => { setPage(1); }, [q, filter, state]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ['law', 'courts', { q, filter, state, page }],
  ...
  const rows = rowsOf<CourtRecord>(data);
  const total = (data as { data?: { pagination?: { total?: number } } })?.data?.pagination?.total ?? rows.length;

  return (
    <div className="space-y-6">
      <PageHeader title="Courts" description="Court profiles shown on lawelitenetwork.com under Legal → Courts. Records here override the website’s built-in ones." actions={<Button asChild><Link href="/law/courts/new"><Plus className="mr-2 h-4 w-4" /> New court</Link></Button>} />
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          ...
          <select className={SELECT} value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="level">
            ...
            {COURT_LEVELS.map((o) => <option key={o}>{o}</option>)}
          </select>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Level</TableHead><TableHead>State</TableHead><TableHead className="text-right">Edit</TableHead></TableRow></TableHeader>
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell><div className="font-medium">{r.name}</div><div className="font-mono text-xs text-muted-foreground">/legal/courts/{r.slug}</div></TableCell>
                    ...
                    <TableCell className="text-right"><Button asChild variant="ghost" size="icon" aria-label={`Edit ${r.name}`}><Link href={`/law/courts/${r.id}`}><Pencil className="h-4 w-4" /></Link></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
