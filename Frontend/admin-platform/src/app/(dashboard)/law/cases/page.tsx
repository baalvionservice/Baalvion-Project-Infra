...
import { casesApi, CASE_STATUSES, rowsOf, type CaseRecord } from '@/lib/law/legal';
...
export default function LawCasesPage() {
  ...
  useEffect(() => { setBreadcrumbs([{ label: 'Law Elite', href: '/law' }, { label: 'Cases' }]); }, [setBreadcrumbs]);
  useEffect(() => { setPage(1); }, [q, filter, state]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ['law', 'cases', { q, filter, state, page }],
  ...
  const rows = rowsOf<CaseRecord>(data);
  const total = (data as { data?: { pagination?: { total?: number } } })?.data?.pagination?.total ?? rows.length;

  return (
    <div className="space-y-6">
      <PageHeader title="Cases" description="Public case profiles shown on lawelitenetwork.com under Legal → Cases. Records here override the website’s built-in ones." actions={<Button asChild><Link href="/law/cases/new"><Plus className="mr-2 h-4 w-4" /> New case</Link></Button>} />
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          ...
          <select className={SELECT} value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="status">
            <option value="">Any status</option>
            {CASE_STATUSES.map((o) => <option key={o}>{o}</option>)}
          </select>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Court</TableHead><TableHead>State</TableHead><TableHead className="text-right">Edit</TableHead></TableRow></TableHeader>
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell><div className="font-medium">{r.case_name}</div><div className="font-mono text-xs text-muted-foreground">/legal/cases/{r.slug}</div></TableCell>
                    <TableCell className="text-muted-foreground">{r.court_slug}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right"><Button asChild variant="ghost" size="icon" aria-label={`Edit ${r.case_name}`}><Link href={`/law/cases/${r.id}`}><Pencil className="h-4 w-4" /></Link></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
