...
import { topicsApi, TOPIC_PILLARS, rowsOf, type TopicRecord } from '@/lib/law/legal';

const SELECT = 'h-9 rounded-md border border-input bg-background px-3 text-sm';
const PAGE = 50;

export default function LawTopicsPage() {
  ...
  useEffect(() => { setBreadcrumbs([{ label: 'Law Elite', href: '/law' }, { label: 'Topics' }]); }, [setBreadcrumbs]);
  useEffect(() => { setPage(1); }, [q, filter, state]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ['law', 'topics', { q, filter, state, page }],
  ...
  const rows = rowsOf<TopicRecord>(data);
  const total = (data as { data?: { pagination?: { total?: number } } })?.data?.pagination?.total ?? rows.length;

  return (
    <div className="space-y-6">
      <PageHeader title="Topics" description="Topic hubs on lawelitenetwork.com. Articles that mention a topic’s name or alias are connected automatically. Records here override the website’s built-in ones." actions={<Button asChild><Link href="/law/topics/new"><Plus className="mr-2 h-4 w-4" /> New topic</Link></Button>} />
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          ...
          <select className={SELECT} value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="Pillar">
            <option value="">Any pillar</option>
            {TOPIC_PILLARS.map((o) => <option key={o}>{o}</option>)}
          </select>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Pillar</TableHead><TableHead>State</TableHead><TableHead className="text-right">Edit</TableHead></TableRow></TableHeader>
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-right"><Button asChild variant="ghost" size="icon" aria-label={`Edit ${r.name}`}><Link href={`/law/topics/${r.id}`}><Pencil className="h-4 w-4" /></Link></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
