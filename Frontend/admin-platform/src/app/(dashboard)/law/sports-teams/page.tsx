...
import { sportsTeamsApi, COMMON_SPORTS, rowsOf, type SportsTeamRecord } from '@/lib/law/legal';

const SELECT = 'h-9 rounded-md border border-input bg-background px-3 text-sm';
const PAGE = 50;

export default function LawSportsTeamsPage() {
  ...
  useEffect(() => { setBreadcrumbs([{ label: 'Law Elite', href: '/law' }, { label: 'Sports teams' }]); }, [setBreadcrumbs]);
  useEffect(() => { setPage(1); }, [q, filter, state]);

  const { data, isLoading, isError } = useQuery({
    queryKey: ['law', 'sports-teams', { q, filter, state, page }],
    queryFn: () => sportsTeamsApi.list({
      page, limit: PAGE,
      ...(q ? { search: q } : {}),
      ...(filter ? { sport: filter } : {}),
  ...
  const rows = rowsOf<SportsTeamRecord>(data);
  const total = (data as { data?: { pagination?: { total?: number } } })?.data?.pagination?.total ?? rows.length;

  return (
    <div className="space-y-6">
      <PageHeader title="Sports teams" description="Team profiles shown on lawelitenetwork.com under Sports → Teams. Records here override the website’s built-in ones." actions={<Button asChild><Link href="/law/sports-teams/new"><Plus className="mr-2 h-4 w-4" /> New team</Link></Button>} />
      ...
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center">
          ...
          <select className={SELECT} value={filter} onChange={(e) => setFilter(e.target.value)} aria-label="sport">
            <option value="">Any sport</option>
            {COMMON_SPORTS.map((o) => <option key={o}>{o}</option>)}
          </select>
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell><div className="font-medium">{r.name}</div><div className="font-mono text-xs text-muted-foreground">/sports/teams/{r.slug}</div></TableCell>
                    ...
                    <TableCell className="text-right"><Button asChild variant="ghost" size="icon" aria-label={`Edit ${r.name}`}><Link href={`/law/sports-teams/${r.id}`}><Pencil className="h-4 w-4" /></Link></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
