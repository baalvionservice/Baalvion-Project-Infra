 ...
 * checked category has that region as its parent is the country. Kept in
 * admin-platform so editors can preview the exact live URL before publishing,
 * without a round trip to the public site.
 */
export function resolveWorldGeo(categoryIds: string[], categories: CategoryRef[]): { region?: string; country?: string } {
...
/**
 * Builds the canonical imperialpedia article path — `/world/<region>/<country>/YYYY/MM/DD/<slug>`
 * when both region and country are tagged, otherwise the flat dated `/YYYY/MM/DD/<slug>`.
...
export function buildImperialpediaPath(params: {
  ...
  const { region, country } = resolveWorldGeo(categoryIds, categories);
