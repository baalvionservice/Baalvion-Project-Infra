...
// legacy /login route stays reachable as a fallback. Jobs is on the .baalvion.com apex, so the
// shared refresh cookie carries the SSO session.
//
// NOTE: this app already uses NEXT_PUBLIC_AUTH_URL for its own auth-service BFF base
// (see lib/authClient.ts), so the shared sign-in surface is configured separately via
