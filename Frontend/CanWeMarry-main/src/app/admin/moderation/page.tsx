...
export const metadata = privateMetadata('Moderation log');
export const dynamic = 'force-dynamic';

export default async function AdminModerationPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const sp = await searchParams;
  const page = Math.max(1, Number(sp.page ?? 1) || 1);
  const result = await moderation.history({ page, pageSize: 50 }, await serverOptions());
  ...
  return (
    <StaffBoundary error={result.ok ? null : result.error}>
      {items.length === 0 ? (
        <EmptyState title="No moderation actions" description="Every action taken on the platform is recorded here with the reason it was taken." />
      ) : (
        <>
